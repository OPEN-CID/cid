...
- **Date**: 2026-07-26
- **Status**: Accepted
- **Context**: Need real PTY per Mission, native cross-platform (not xterm.js-only client-side emulation). Build Prompt Part B suggests `portable-pty` or equivalent with Windows ConPTY and Unix support. Warp, Herdr, cmux all use native PTY with real process behind it.
...
- **Consequences**:
  - `create_pty` takes workdir from mission's worktree_path or repo path — terminals start in correct repo context
