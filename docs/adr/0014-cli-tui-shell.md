...
TUI (e.g., via `ratatui`) — letting a developer drive Sessions, review diffs, approve/deny
...
1. **State (Sessions, repo channels, chat history): plain HTTP polling** of `/api/rpc`,
...
2. **Pending tool-call approvals: the existing `/ws` WebSocket.** This is the one place
   ...
   approval is *only* ever announced as a `session.tool_call.request` push notification,
...
Layout mirrors the desktop/web shells' shape (Part 19) at terminal scale: a Session list,
...
- **Verified against a live Core**, not just unit tests: connected to a running Core over
  HTTP, fetched a real repo channel and Session, and rendered them correctly in the
  terminal frame buffer.
- Approval visibility is scoped to whichever Session is currently selected — switching
  Sessions clears the pending-approval list rather than tracking all Sessions
  simultaneously. A developer running many parallel autonomous Sessions and wanting a
