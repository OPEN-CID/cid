...
| `cid-tui` | A `ratatui`-based terminal client (`src/main.rs`, `api.rs`, `app.rs`, `events.rs`, `ui.rs`) — session list, chat, tool-call/plan approval, over the same HTTP+WebSocket JSON-RPC API the desktop/web shells use. | The CLI-first persona (Part 34 of the Phase 5 prompt): a developer who wants to drive a Session without a GUI. It is a client, not a fork of Core's logic — it holds no business logic beyond rendering and input handling. |
