...
async fn non_vibe_session_still_uses_the_full_planner() {
    ...
    // Pin the Planner to an endpoint that refuses instantly.
    //
    // `start_core` inherits the developer's environment, so a machine with
    // ANTHROPIC_API_KEY/GEMINI_API_KEY set made this test issue *live* provider
    // calls — and its timing then depended on that vendor's current capacity.
    // It passed on a 4s budget only while the default model 404'd immediately
    // on a retired id; once that was fixed the Planner legitimately took
    // longer, and longer again when falling back past a model returning 503
    // (observed: 26s for one plan). What this test actually asserts — that the
    // non-vibe path does not stamp the vibe-preset marker — needs a plan to
    // exist, not a good one, so a fast, deterministic failure is exactly right.
    rpc_ok(
        &base,
        "settings.update",
        json!({
            "planner_provider": "openai_compatible",
            "openai_compatible_endpoint": "http://127.0.0.1:1/v1",
        }),
    )
    .await;
    ...
    panic!("expected a plan to be generated by the background Planner within 20s");
}
